...
section .text
_main:
    ; gettimeofday(&timeval, NULL) — get UTC epoch seconds
    sub  rsp, 16                ; timeval: tv_sec(8) + tv_usec(8)
    mov  eax, 0x02000074        ; syscall: gettimeofday
    lea  rdi, [rsp]
    xor  esi, esi
    syscall
...
.format:
    ...
    mov  eax, 0x02000004        ; syscall: write (macOS)
    mov  edi, 1                 ; fd: stdout
    lea  rsi, [buf]
    mov  edx, BUF_LEN
    syscall
    ...
    ; exit(0)
    mov  eax, 0x02000001        ; syscall: exit (macOS)
    xor  edi, edi
    syscall
