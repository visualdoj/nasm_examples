...
bDurNs            equ 0        ; qword: seconds * 1000000000
bStartNs          equ 8        ; qword: flat nanoseconds at start
...
msgbuf          resb 80
termiosBuf      resb 64
nowTs           resq 2          ; { tv_sec (qword), tv_nsec (qword) }
...
main:
    ...
    sub  rsp, 8                ; align to 16: (8-8) mod 16 = 0
...
.stderr_not_tty:
    ...
    jle  .exit_err             ; 0 or negative => too few args
    cmp  r12d, MAX_BARS
    jg   .exit_err             ; too many args
    mov  [num_bars], r12d
...
.init:
    ...
    mov  r15, [r13 + rbx*8 + 8]   ; r15 = argv[ebx+1]
    xor  ebp, ebp                  ; ebp = seconds accumulator
.prs:
    movzx ecx, byte [r15]
    test  cl, cl
    jz    .prs_end
...
.prs_end:
    ...
    ; bar.durationNs = ebp * 1000000000
    mov  rax, 1000000000
    imul rax, rbp
    mov  [r14 + bDurNs], rax

    ; bar.startNs = clock_gettime(CLOCK_MONOTONIC) as flat nanoseconds
...
.main_loop:
    ...
    ; Sample current time as flat nanoseconds into r15
    mov  eax, SYS_clock_gettime
    mov  edi, CLOCK_MONOTONIC
    lea  rsi, [nowTs]
    syscall
...
.scan_run:
    ; pct = elapsed * 100 / durationNs
    xor  edx, edx
    imul rax, 100
    div  qword [r14 + bDurNs]
...
.cmsg:
    ...
    ; TTY: move cursor down past bars + previous messages, then back up after
    cmp  qword [stderr_is_tty], 0
    je   .cmsg_build
...
.i2s:
    ...
    ; (num_bars + msg_count + 1) lines: +1 because message LF advanced cursor
    cmp  qword [stderr_is_tty], 0
    je   .cmsg_skip_up
